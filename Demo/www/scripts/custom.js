...
//        }
        ...
        function search_menu(){
            $('[data-search]').on('keyup', function() {
                ...
                if (searchVal != '') {
                    ...
                    $.ajax({
                        url: 'http://torrance.fitandappy.com/search.php',
                        method: 'get',
                        data: {search: searchVal, spage: sPage},
                        cache: false,
                        dataType: 'json',
                        ...
                        $.each(data, function (k, val) {
                            ...
                            var v_img = '';
                            if (res == 'pdf') {
                                v_img = "http://torrance.fitandappy.com/admin_cta/images/pdf_default.jpg";
                            } else if (res == 'doc' || res == 'docx') {
                                v_img = "http://torrance.fitandappy.com/admin_cta/images/word_default.jpg";
                            } else if (res == 'jpg' || res == 'png' || res == 'jpeg') {
                                v_img = "http://torrance.fitandappy.com/admin_cta/upload_files/" + val['name'];
                            } else {
                                v_img = "http://torrance.fitandappy.com/admin_cta/images/default.jpg";
                            }

                            content += '<div data-filter-item data-filter-name="all ' + val['name'] + '"  class="search-result-list">';
                            content += '<img class="shadow-large" src="' + v_img + '" alt="img" style="height:50px; width:50px">';
                            content += '<h1>' + val['name'] + '</h1>';
                            content += '<p></p>';
                            content += '<a href="http://torrance.fitandappy.com/admin_cta/upload_files/' + val['name'] + '" class="bg-highlight" target="_blank">VIEW</a></div>';
                        });


                        $('.search-results-items').html(content);
       ...
       $(".text-size-default").click(function() {$(".text-size-changer *").css("font-size", "");});
        ...
        /*===================== index begin  =====================*/
        if (href.search('index.html') != -1 || href.endsWith('android/') || href.endsWith('android')) {
            // new page items
            $.ajax({
                url: 'http://torrance.fitandappy.com/new_pages.php',
                dataType: 'json'
            }).done(function (result) {
                var $wrapper = $('#newPageContents');
                var html = '';

                $.each(result, function (k, val) {
                    if (k % 2 == 0) {
                        html += '<div class="content"><div class="one-half">';
                    } else {
                        html += '<div class="one-half last-column">';
                    }
                    ...
                    html += '<a class="caption round-medium shadow-huge bottom-20" data-height="150" href="' + url + '" style="height:150px;">';
                    html += '<div class="caption-center"><h4 class="center-text color-white bottom-10">';
                    html += '<i class="fa-2x ' + val['icon'] + '"></i></h4>';
                    html += '<h1 class="boxed-text-huge under-heading color-white opacity-90 bottom-15">' + val['name'] + '</h1></div>';
                    html += '<div class="caption-overlay bg-blue2-dark opacity-80"></div><div class="caption-bg"></div></a>';
        ...
        /*===================== template_maps begin  =====================*/
        if (href.search('template_maps.html') != -1) {
            var id = getUrlParameter('id');
            if (id) {
                var data;
                $.ajax({ // ajax call starts
                    url: 'http://torrance.fitandappy.com/template.php',
                    data: {id: id},
                    dataType: 'json'
                }).done(function (result) {
                    ...
                    var content = '';
                    data = result['data'];
                    $.each(data, function (k, val) { // $ext = val['name'];
                        content = content + "<div class='content-boxed map-item' data-index='" + i + "'>" +
                            "<div class='item text-center'>" +
                            "<div style=\"display:inline-block;width:350px;height:200px;\">" + val['map'] +
                            "</div>" + "</div>" + "</div>";
                        i++;
                    });
                    ...
                    $('.map-item').click(function() {
                        ...
                        myWindow.document.write(data[index]['map']);
                    });
                });       
            }
        }
        ...
        if (href.search('template_files.html') != -1) {
            var id = getUrlParameter('id');
            if (id) {
                $.ajax({
                    url: 'http://torrance.fitandappy.com/template.php',
                    ...
                    $.each(data, function (k, val) {
                        ...
                        var v_img = '';
                        if (res == 'pdf') {
                            v_img = "<img style='height:50px; width:50px' src='http://torrance.fitandappy.com/admin_cta/images/pdf_default.jpg'>";
                        } else if (res == 'doc' || res == 'docx') {
                            v_img = "<img style='height:50px; width:50px' src='http://torrance.fitandappy.com/admin_cta/images/word_default.jpg'>";
                        } else if (res == 'jpg' || res == 'png' || res == 'jpeg') {
                            v_img = "<img style='height:50px; width:50px' src='http://torrance.fitandappy.com/admin_cta/upload_files/" + val['name'] + "'>";
                        } else {
                            v_img = "<img style='height:50px; width:50px' src='http://torrance.fitandappy.com/admin_cta/images/default.jpg'>";
                        }
                        // $ext = val['name'];
                        content = content + "<div class='content-boxed'>" + "<div class='item'>" + "<div class='item-img'>" + v_img + "</div>" + "<div class='item-info'>" + "<div>" + "<p class='item-time'><span style='font-size:11px'>" + val['name'] + "</span></p>" + "</div>" + "</div>" + "<div>" + "<div>" + "<p class='item-time' style='margin-right:12px;'><a href='http://torrance.fitandappy.com/admin_cta/upload_files/" + val['name'] + "' target='_blank'><span id='unitedBrokers' style='font-size:20px;color:blue;cursor:pointer;display:block;' onmouseover=\"this.style.color='red'\" onmouseout=\"this.style.color='red'\" ><i class=\"fas fa-cloud-download-alt\"></i></span></a></p>" + "</div>" + "</div>" + "</div>" + "</div>";
                    });
        ...
        /*===================== template_form begin  =====================*/
        if (href.search('template_form.html') != -1) {
            var id = getUrlParameter('id');
            if (id) {
                $.ajax({
                    url: 'http://torrance.fitandappy.com/template.php',
                    ...
                    data = result['data'];
                    $.each(data, function (k, val) {
                        var sub_title = val['title'];
                        var sub_description = val['description'];
                        $('#about_title').html(sub_title);
                        $('#about_description').html(sub_description);
                    });
    ...
    if ($('#footerWrapper').length > 0) {
        $('#footerWrapper').load('footer.html', function() {
